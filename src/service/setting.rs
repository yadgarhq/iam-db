...
impl IamDb {
    pub(super) async fn store_setting(
        ...
        if claim.already_recorded(&mut tx).await? {
            // **NOTHING IS WRITTEN ON THIS PATH.** The verb is a state-setter, so
            // re-running it would reach the same state — but re-running it after
            // somebody else legitimately changed the level would UNDO their change
            // and report success, which is the silent overwrite the ledger exists
            // to stop.
            //
            // THE OUTCOME IS RE-DERIVED RATHER THAN STORED, and the contract says
            // why: this RPC is not in ADR-0519's single-use-secret carve-out,
            // because the outcome is the stored setting and the store keeps it.
            // There is nothing spent to hand back.
            return answer(tx, &r.name, call, 0).await;
        }

        let (done, call) = write_at_scope(&mut tx, &r, scope, call).await?;
        ...
        record_claim(&mut tx, &claim).await?;
...
///
/// **THE `Call` GOES IN AND COMES BACK.** The unreachable-scope arm refuses, and
/// `Call::fail` consumes the call; every other arm leaves the RPC with work still
/// to do. Handing it back is what keeps that arm's refusal exactly where it was
...
async fn write_at_scope(
    ...
    scope: SettingScope,
    call: Call,
) -> Result<(sqlx::mysql::MySqlQueryResult, Call), Status> {
    let done = match (scope, r.clear) {
        ...
        (SettingScope::Team, false) => set_team_override(&mut *tx, r).await?,
        ...
        (SettingScope::Unspecified, _) => {
            let refusal = Status::invalid_argument("scope names no level this contract declares");
            call.fail(label(&refusal));
            return Err(refusal);
        }
    };
...
/// The team-scope set, whose liveness predicate rides IN the write.
///
/// One function because the `INSERT ... SELECT ... LOCK IN SHARE MODE` and the
/// `live_team` that names its zero are one mechanism: the clause is what makes
...
/// D9's ledger row for this write, and the branch where a concurrent delivery
/// recorded one first.
///
/// Lifted out whole: the duplicate-key branch is correct only together with the
/// INSERT that raises it, so neither half is a function on its own.
async fn record_claim(tx: &mut sqlx::MySqlTransaction<'_>, claim: &Claim) -> Result<(), Status> {
    if !claim.key.is_empty() {
        if let Err(e) = claim.record(tx).await {
            let duplicate = e
                .as_database_error()
            ...
            let prior = recorded(tx, &claim.key, Lock::Yes).await?.ok_or_else(|| {
                ...
                tracing::error!("a duplicate idempotency key vanished before it could be re-read");
...
/// THE SETTING WHOLE, read in the transaction that wrote it, and the end of the
/// RPC either way.
///
/// NOT the echo D48 refuses: the caller sent one level, and what goes back is the
/// other level and every OTHER team's override — which the caller did not send
/// and has no administrative read verb to fetch.
///
/// `rows` is the one thing the two callers differ on. A replay wrote nothing and
/// says zero; the write says what it changed.
async fn answer(
    ...
    let setting = read_inherited_setting(&mut tx, name).await?;
