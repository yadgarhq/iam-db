...
| `DB_SSL_CA_FILE`                                                | unset                                | the certificate authority `verify_ca` and `verify_identity` check the engine's certificate against. Unset means sqlx's own trust store — the PUBLIC web roots, which sign no operator-issued, RDS or Aurora certificate — so a verifying mode without this is either refused or, under `verify_ca`, accepts any publicly-trusted certificate for any name. Inert under the three non-verifying modes |
